...
export type SidebarContainerConfig = {
  ...
  /**
   * Where to place the sidebar UI. Defaults to 'right'.
   * If 'bottom', render the sidebar as a fixed bottom overlay panel.
   */
...
export class Sidebar implements Destroyable {
  ...
  constructor(
    ...
    if (config.externalContainerSelector) {
      ...
      const isBottom = config.sidebarPlacement === 'bottom';
      this.iframeContainer.className = isBottom
        ? 'sidebar-container-bottom'
        : 'sidebar-container';
      this.iframeContainer.id = iframeContainerId;
  if (config.theme === 'clean' && !isBottom) {
      ...
      // Bottom overlay: toolbar will be created after the shadow root is set up
      // to avoid referencing this._hypothesisSidebar before initialization.
    }
      ...
      // Bottom overlay placement: style the container as a fixed bottom panel
      // and add a backdrop and fixed toolbar inside the same shadow root.
      if (this._config.sidebarPlacement === 'bottom') {
        const frame = this.iframeContainer;
        // Fixed bottom panel covering ~40% viewport height
  ...
  frame.style.maxHeight = '80vh';
        // Clear any transform coming from sidebar CSS defaults
        (frame.style as any).transform = '';
  frame.style.width = 'auto';
  frame.style.marginLeft = '0';
        frame.style.zIndex = '2147483647';
  // Panel content (iframe) slides from bottom; toolbar remains visible
  this.iframe.style.display = 'block';
  this.iframe.style.transform = 'translateY(calc(100% + 8px))';
  this.iframe.style.transition = 'transform 200ms ease-out';
  // Prevent closed panel from intercepting clicks
  frame.style.pointerEvents = 'none';
        ...
        // Fixed toolbar at bottom-left, appended to shadow root now that it exists
        const toolbarFixed = document.createElement('div');
        toolbarFixed.setAttribute('data-testid', 'toolbar-fixed');
        toolbarFixed.style.position = 'fixed';
        toolbarFixed.style.left = '0';
        toolbarFixed.style.bottom = '0';
        toolbarFixed.style.zIndex = '2147483648';
        // Allow pointer events in the toolbar
        toolbarContainer.className = 'pointer-events-auto';
        toolbarFixed.append(toolbarContainer);
        shadowRoot.appendChild(toolbarFixed);
      }
    ...
    // Close overlay with Escape on bottom placement
    if (this._config.sidebarPlacement === 'bottom') {
      this._listeners.add(document, 'keydown', e => {
        const evt = e as KeyboardEvent;
        if (this.toolbar.sidebarOpen && evt.key === 'Escape') {
          evt.stopPropagation();
          this.close();
        }
      });
    ...
    if (toggleButton && this._config.sidebarPlacement !== 'bottom') {
  ...
  _updateLayoutState(expanded?: boolean) {
    ...
    // For bottom placement, report no pushing width (toolbar only) so guests
    // do not switch to side-by-side mode. Height reflects the overlay height.
    if (this._config.sidebarPlacement === 'bottom') {
      const isExpanded = typeof expanded === 'boolean' ? expanded : this.toolbar.sidebarOpen;
  ...
  _onResize() {
    if (this._config.sidebarPlacement === 'bottom') {
      // No responsive auto-close behavior for bottom overlay
      return;
    }
  ...
  this.iframe.style.transform = 'translateY(0)';
        ...
        if (this._backdropElement) {
          this._backdropElement.style.opacity = '1';
